...
class MaLibrary:
    def __init__(self, name, got_mappings, data, data_length, entry_address, key, elf_offset_key="0x1a20"):
        self.name = name
...
# prepare the working-/outputdirectory
dir = f"{cwd}/out"
...
def pack(module):
    ...
    main = "loader.c"
    out = f"{dir}/malworm.c"
    mainfile = open(f"{cwd}/{main}", "r")
    code = mainfile.read()
    mainfile.close()

    # copy globals.h to output directory so it is available for compilation
    bash(f"cp globals.h {dir}")

    # define all the filenames
    c_file = f"{module}.c"
    o_file = f"{dir}/object.o"
    so_file = f"{dir}/{module}.so"
    
    # compile the library as position-independent shared object
    bash(f"{gcc} -c -fPIC -o {o_file} {c_file}", verbose=True)    
    bash(f"{gcc} -shared -o {so_file} {o_file}", verbose=True)    
